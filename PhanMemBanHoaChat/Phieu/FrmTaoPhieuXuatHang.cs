...
{
    public partial class FrmTaoPhieuXuatHang : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FrmTaoPhieuXuatHang()
        {
            InitializeComponent();
        }
        public FrmTaoPhieuXuatHang(string tendn)
        {
            InitializeComponent();
            LoadDataKH();
            LoadDataHH();
            ...
            txbmanv.Text = maNV;
            txbmapxh.Text = TaoRandomMaPXH();
        }

        private void FrmTaoPhieuXuatHang_Load(object sender, EventArgs e)
        {
        ...
        public void ChonHangFromDataGridView(string maPXH)
        {
            PXHCTiet PXH = pxh.GetDataPXHang(maPXH);

            if (PXH != null)
            {
                txbmapxh.Text = PXH.MaPXH;
                DateTime ngayLap = Convert.ToDateTime(PXH.NgayLap);
                lbhienngayl.Text = ngayLap.ToString("dd/MM/yyyy");
                dtpNgayG.Value = Convert.ToDateTime(PXH.NgayGiao);
                txbtennv.Text = PXH.TenNV;
                txbmanv.Text = PXH.MaNV;
                cbbtenkh.Text = PXH.TenKH;
                cbbtenkh.ForeColor = Color.Black;
                txbmakh.Text = PXH.MaKH;
                txbdcgkh.Text = PXH.DiaChiGiao;
                txbtongtt.Text = PXH.TongThanhTien.Value.ToString("#,##0");
                txbvat.Text = PXH.Thue.Trim().TrimEnd('%');
                txbtongtientt.Text = PXH.TongCongThanhToan.Value.ToString("#,##0");
                txbghichu.Text = PXH.GhiChu;
                List<CTPXHCTiet> ctpxhList = pxh.GetDataCTPXHang(maPXH);
                if (ctpxhList != null && ctpxhList.Any())
                {
                    foreach (CTPXHCTiet ctpxhItem in ctpxhList)
                    {
                        string maHH = ctpxhItem.MaHangHoa;
                        string tenHH = ctpxhItem.TenHangHoa;
                        string dvt = ctpxhItem.DonViTinh;
                        int? sl = ctpxhItem.SoLuong;
                        string quyC = ctpxhItem.QuyCach;
                        string donG = ctpxhItem.DonGia.HasValue ? ctpxhItem.DonGia.Value.ToString("#,##0") : "";
                        string thanhT = ctpxhItem.ThanhTien.HasValue ? ctpxhItem.ThanhTien.Value.ToString("#,##0") : "";
        ...
        private string TaoRandomMaPXH()
        {
            ...
            do
            {
                ...
                maPXH = "PXH" + randomValue.ToString("000");
            } while (KiemTraTrungMaPXH(maPXH));

            return maPXH;
        }

        private bool KiemTraTrungMaPXH(string maPXH)
        {
            return false;
        }
        ...
        private void btnTao_Click(object sender, EventArgs e)
        {
            ...
            string TTXuatHang = "Yêu cầu xuất hàng";
            ...
            if (dataGridView1.RowCount > 1 || (dataGridView1.RowCount == 1 && dataGridView1.Rows[0].Cells[0].Value != null))
            {
                if (cbbtenkh.SelectedItem != null)
                {
                    if (ngayG >= ngayL)
                    {
                        pxh.themPXH(txbmapxh.Text, ngayL, DateTime.Parse(dtpNgayG.Text), Convert.ToDouble(txbtongtt.Text), VAT, Convert.ToDouble(txbtongtientt.Text), TTXuatHang, txbghichu.Text, txbmakh.Text, txbmanv.Text, maHangHoaList.ToArray(), soLuongList.ToArray(), quycList.ToArray(), dgList.ToArray(), ttList.ToArray());
        ...
        private void btnXuatHoaDon_Click(object sender, EventArgs e)
        {
            ...
            List<CTPXHCTiet> List = new List<CTPXHCTiet>(pxh.GetDataCTPXHang(txbmapxh.Text));
            try
            {
                string fileName = "BaoCao.xls";
                if (ee.ExportPhieuXuatHang(List, ref fileName, true, txbdvb.Text, txbdiachi.Text, txbemail.Text,txbmapxh.Text, cbbtenkh.Text, txbdcgkh.Text, txbmakh.Text, dtpNgayG.Text, txbtongtt.Text, txbvatdatinh.Text, txbtongtientt.Text))
                {
